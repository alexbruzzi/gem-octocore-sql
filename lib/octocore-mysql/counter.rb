require 'octocore-mysql/counter/helpers'
...
module Octo
  module Counter
    ...
    def increment_for(obj)
      ...
      if obj.class.ancestors.include?ActiveRecord::Base
        args = obj.key_attributes.collect { |k,v|  v.to_s }
        cache_key = generate_key(ts, obj.class.name, *args)

        val = ActiveRecord::Base.redis.get(cache_key)
        if val.nil?
          val = 1
        else
          val = val.to_i + 1
        ...
        ActiveRecord::Base.redis.setex(cache_key, ttl, val)

        # Optionally, update the index
        index_key = generate_index_key(ts, obj.class.name, *args)
        index_present = ActiveRecord::Base.redis.get(index_key).try(:to_i)
        if index_present != 1
          ActiveRecord::Base.redis.setex(index_key, ttl, 1)
        end
      end
    ...
    def aggregate(ts = Time.now.floor)
      ...
      counters = ActiveRecord::Base.redis.keys(index_key)
      counters.each do |cnt|
        ...
        counter_keys = ActiveRecord::Base.redis.keys(counters_search_key)
        counter_keys.each do |c_key|
          val = ActiveRecord::Base.redis.get(c_key)
          if val
